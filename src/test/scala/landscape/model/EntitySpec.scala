...
import landscape.common.UUIDConversions
 ...
 */
case class User(val uuidOption: Option[UUID], name: String) extends Entity[User] {
  def withUuid(uuid: UUID): User = copy(uuidOption = Some(uuid))
}
...
class EntitySpec extends WordSpec with Matchers {
  ...
  "Entity" should {
    ...
    "set its uuid" in {
      ...
      val uuid = UUIDConversions.generate

      User(None, "John").withUuid(uuid).uuidOption shouldEqual Some(uuid)
